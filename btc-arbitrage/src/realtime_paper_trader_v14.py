#!/usr/bin/env python3
"""
Real-time Paper Trader v14 - Probability-First Trading
Based on V12, with fundamental improvements:
1. Probability scoring before every entry (ProbabilityScorer module)
2. EV-based entry gating (only enter if EV > MIN_EV)
3. Fixed VOL_REGIME filter: BTC 0.50%, ETH 0.80% (soft filter, not hard block)
4. Fixed stop-loss: ATR_STOP 2.0x ATR minimum, floors $0.15 BTC / $0.10 ETH
5. Fixed flash_sniper SL: 15% (was 10%), time-based stop relaxation
6. Comprehensive EV logging for every entry decision

V14 = V12 + Probability Scoring + Fixed Filters

Key principle (KISS): Simple weighted probability scoring, not ML.
All V12 strategies preserved: steam_follow, flash_sniper, settlement_rush, etc.
"""
...
# Import probability scorer
from probability_scorer import ProbabilityScorer

# V14 Configuration
MIN_EV = 0.10  # Minimum expected value to enter a trade ($)
VOL_REGIME_BTC = 0.005  # 0.50% ATR threshold for BTC (was 0.0025)
VOL_REGIME_ETH = 0.008  # 0.80% ATR threshold for ETH (was 0.0025)
ATR_STOP_MULTIPLIER = 2.0  # ATR stop multiplier (was 1.0-1.5x)
ATR_STOP_FLOOR_BTC = 0.15  # Minimum stop-loss for BTC contracts ($)
ATR_STOP_FLOOR_ETH = 0.10  # Minimum stop-loss for ETH contracts ($)
FLASH_SNIPER_SL_PCT = 0.15  # Flash sniper stop-loss % (was 0.10)
...
class TradingEngine:
    def __init__(self, feed, poller, okx_feed, duration_min=480):
        self.feed = feed
        self.poller = poller
        ...
        self.state_file = '/home/clawdbot/clawd/btc-arbitrage/data/rt_v14_state.json'
        self.graceful_shutdown = False
        
        # V14: Probability scorer
        self.scorer = ProbabilityScorer()
        self.ev_log = []  # Track all EV decisions for analysis
    ...
    def _gather_indicators(self, market_type):
        """V14: Gather all available indicators for the probability scorer."""
        asset_type = 'BTC' if market_type in ('15m', 'btc_daily') else 'ETH'
        indicators_obj = self.feed.btc_indicators if asset_type == 'BTC' else self.feed.eth_indicators
        
        indicators = {}
        
        # Momentum at multiple timeframes
        if asset_type == 'BTC':
            indicators['momentum_1m'] = self.feed.get_momentum(60)
            indicators['momentum_5m'] = self.feed.get_momentum(300)
            indicators['momentum_15m'] = self.feed.get_momentum(900)
            indicators['current_price'] = self.feed.weighted_price
        else:
            indicators['momentum_1m'] = self.feed.get_eth_momentum(60)
            indicators['momentum_5m'] = self.feed.get_eth_momentum(300)
            indicators['momentum_15m'] = self.feed.get_eth_momentum(900)
            indicators['current_price'] = self.feed.eth_weighted_price
        
        # Technical indicators
        indicators['rsi'] = indicators_obj.get_rsi()
        indicators['ema_trend'] = indicators_obj.get_ema_trend()
        indicators['ema_5'] = indicators_obj.ema_5
        indicators['ema_20'] = indicators_obj.ema_20
        indicators['bb_upper'] = indicators_obj.bb_upper
        indicators['bb_lower'] = indicators_obj.bb_lower
        indicators['bb_middle'] = indicators_obj.bb_middle
        
        # OKX data
        indicators['ls_ratio'] = self.okx_feed.get_long_short_ratio(asset_type)
        
        # Time remaining
        time_remaining = self.poller.time_until_close(market_type)
        indicators['time_remaining_sec'] = time_remaining if time_remaining < 9999 else None
        
        # Volume ratio (approximate from OKX OI changes or Kalshi volume)
        # Simple: compare current Kalshi volume to expected
        kalshi = self.poller.markets.get(market_type, {})
        kalshi_vol = kalshi.get('volume', 0)
        indicators['volume_ratio'] = min(kalshi_vol / 200.0, 5.0) if kalshi_vol > 0 else None  # Normalize against ~200 avg
        
        return indicators
    
    def _check_ev(self, market_type, direction, kalshi_price, strategy, elapsed):
        """
        V14: Check EV before entry. Returns (should_enter, ev_result, prob_result).
        Logs the decision regardless.
        """
        asset_type = 'BTC' if market_type in ('15m', 'btc_daily') else 'ETH'
        indicators = self._gather_indicators(market_type)
        
        # Get probability estimate
        prob_result = self.scorer.estimate_probability(indicators, asset_type)
        prob_up = prob_result['prob_up']
        confidence = prob_result['confidence']
        
        # Calculate EV
        ev_result = self.scorer.calculate_ev(prob_up, direction, kalshi_price)
        ev = ev_result['ev']
        edge = ev_result['edge']
        ...
        self.ev_log.append(ev_record)
        
        # Log the decision
        action = "ENTER" if ev >= MIN_EV else f"SKIP (ev=${ev:.2f})"
        self._dual_log(
            f"[{elapsed:5d}s] [EV] market={market_type} strategy={strategy} dir={direction} "
            f"prob_up={prob_up:.3f} conf={confidence:.2f} kalshi={kalshi_price:.2f} "
            f"ev=${ev:.3f} edge={edge:+.3f} → {action}"
        )
        
        return ev >= MIN_EV, ev_result, prob_result
    
    def _get_vol_regime_modifier(self, market_type):
        """
        V14: Soft volatility regime filter. Returns position size multiplier (0.5-1.0).
        High vol → reduce position size instead of blocking entirely.
        """
        asset_type = 'BTC' if market_type in ('15m', 'btc_daily') else 'ETH'
        ...
        if atr is None:
            return 1.0  # No data, full size
        
        threshold = VOL_REGIME_BTC if asset_type == 'BTC' else VOL_REGIME_ETH
        
        if atr <= threshold:
            return 1.0  # Normal vol, full size
        elif atr <= threshold * 1.5:
            return 0.75  # Slightly elevated vol, reduce to 75%
        elif atr <= threshold * 2.0:
            return 0.50  # High vol, reduce to 50%
        else:
            return 0.50  # Very high vol, still allow at 50% (never fully block)
    ...
    def _save_checkpoint(self):
        try:
            ...
            state = {
                ...
                'ev_decisions': len(self.ev_log),
                'ev_entries': sum(1 for e in self.ev_log if e['entered']),
                'ev_skips': sum(1 for e in self.ev_log if not e['entered']),
            }
            with open(self.state_file, 'w') as f:
                json.dump(state, f, indent=2, default=str)
            self._dual_log(f"[{elapsed:5d}s] 💾 Checkpoint (v14)")
        except:
            pass
    ...
    def _should_enter(self, market_type, elapsed, strategy=None):
        ...
        # Day 1 Fix: Kill flash_sniper strategy entirely (6.7% win rate — signal is noise)
        if strategy == 'flash_sniper':
            return False, "flash_sniper_KILLED_day1"
        ...
        # V14: Volatility Regime Filter — SOFT (was hard block in V12 at 0.25%)
        # Now uses asset-specific thresholds and reduces size instead of blocking
        asset_type = 'BTC' if market_type in ('15m', 'btc_daily') else 'ETH'
        indicators = self.feed.btc_indicators if asset_type == 'BTC' else self.feed.eth_indicators
        atr = indicators.get_atr()
        vol_threshold = VOL_REGIME_BTC if asset_type == 'BTC' else VOL_REGIME_ETH
        
        if atr is not None:
            if atr > vol_threshold * 2.0:  # Only block at EXTREME vol (2x the already-raised threshold)
                return False, f"🚫 VOL_REGIME_EXTREME(ATR={atr*100:.2f}%>{vol_threshold*200:.1f}%)"
            # Note: moderate high vol is handled by _get_vol_regime_modifier() reducing position size
    ...
    def _get_stop_dollar(self, entry_price, strategy=None, market_type=None):
        """V14: ATR-based adaptive stop-loss with higher floors (V12 was too tight)"""
        ...
        # V14: Asset-specific stop-loss floors
        stop_floor = ATR_STOP_FLOOR_BTC if asset_type == 'BTC' else ATR_STOP_FLOOR_ETH
        
        if atr is not None:
            # V14: Base stop = 2.0 × ATR (was 1.5x — too tight, caused 15.4% win rate)
            atr_stop = atr * ATR_STOP_MULTIPLIER
            
            # Check volatility regime and widen further in high vol
            vol_threshold = VOL_REGIME_BTC if asset_type == 'BTC' else VOL_REGIME_ETH
            if atr > vol_threshold:
                atr_stop = atr * (ATR_STOP_MULTIPLIER + 0.5)  # 2.5x in high vol
            
            # V14: Higher floor ($0.15 BTC, $0.10 ETH) and higher ceiling
            stop_dollar = max(stop_floor, min(0.25, atr_stop))
        ...
        # V14: Higher base stops
        base_stop = 0.12  # Was 0.08
        
        # Steam trades: widened stops to survive noise — BTC $0.18, ETH $0.15
        if strategy == 'steam_follow':
            base_stop = 0.15 if market_type in ('eth_15m', 'eth_daily') else 0.18
        
        # Settlement rush: moderate stop ($0.08) — should resolve fast but not too tight
        if strategy == 'settlement_rush':
            base_stop = 0.08
        ...
        # V14: Higher floor and ceiling
        return max(stop_floor, min(0.25, stop))
    ...
    def _process_market(self, market_type, elapsed, brti, momentum_1m):
        ...
        if is_flash_crash:
            ...
            if not flash_entry_blocked:
                ...
                # V14: Use asset-specific vol thresholds (was hardcoded 0.0025)
                flash_vol_threshold = (VOL_REGIME_BTC if asset_type == 'BTC' else VOL_REGIME_ETH) * 2.0
                if atr is not None and atr > flash_vol_threshold:
                    flash_entry_blocked = True
                    flash_block_reason = f"🚫 VOL_REGIME (flash, ATR={atr*100:.2f}%)"
            ...
            if not flash_entry_blocked:
                ...
                # V14: EV check before flash sniper entry
                ev_pass, ev_result, prob_result = self._check_ev(market_type, direction, entry, 'flash_sniper', elapsed)
                if not ev_pass:
                    flash_entry_blocked = True
                    flash_block_reason = f"EV_NEGATIVE(ev=${ev_result['ev']:.3f})"
                ...
                if len(parts) >= 3:
                    ...
                    if spread < 0.05:  # Spread < 5¢
                        if underlying_price > strike_price and implied_prob > 0.6:
                            ...
                            entry = k_ask
                            direction = 'YES'
                            # V14: EV check before entry
                            ev_pass, ev_result, prob_result = self._check_ev(market_type, direction, entry, 'settlement_rush', elapsed)
                            if not ev_pass:
                                pass  # EV logging already done in _check_ev
                            else:
                                pos_id = self.trader.open(direction, entry, ticker, market_type, 'settlement_rush')
                                if pos_id is not None:
                                    # V14: Apply vol regime position size modifier
                                    vol_mod = self._get_vol_regime_modifier(market_type)
                                    if vol_mod < 1.0:
                                        pos = self.trader.positions[-1]
                                        original_size = pos['size']
                                        pos['size'] = max(5, int(pos['size'] * vol_mod))
                                        self._dual_log(f"[{elapsed:5d}s] 📉 VOL_REGIME modifier {vol_mod:.0%}: size ${original_size} → ${pos['size']}")
                                    ...
                                    self._dual_log(f"[{elapsed:5d}s] 🚀 {market_type.upper()} SETTLEMENT_RUSH: {asset_name} ${underlying_price:,.0f} vs ${strike_price:,.0f} → {direction}@{entry:.2f} (T-{time_until_close:.0f}s) ev=${ev_result['ev']:.3f}")
                                    self.signals.append({'type': 'settlement_rush', 'market': market_type, 'time': elapsed, 'ev': ev_result['ev']})
        ...
        # STRATEGY 3: Steam Move Detection - Smart money following (V11: with trend filter, V12: enhanced)
        # Day 1 Fix: Disable ETH steam_follow (23% WR — negatively correlated). BTC-only.
        elif time_until_close > settlement_window and market_type in ('15m', 'btc_daily'):
            ...
            if steam_result['steam_detected'] and steam_result['direction']:
                ...
                if 0.15 <= entry <= 0.85:
                    # V14: EV check before entry
                    ev_pass, ev_result, prob_result = self._check_ev(market_type, direction, entry, 'steam_follow', elapsed)
                    if not ev_pass:
                        self.last_kalshi_bids[market_type] = kalshi['yes_bid']
                        return
                    ...
                    if pos_id is not None:
                        ...
                        self._dual_log(f"[{elapsed:5d}s] 🔥 {market_type.upper()} STEAM: K price jumped {steam_desc} → {direction}@{entry:.2f} (MTF={mtf_ratio:.1%}) ev=${ev_result['ev']:.3f}")
                        self.signals.append({'type': 'steam_follow', 'market': market_type, 'time': elapsed, 'ev': ev_result['ev']})
        ...
        # STRATEGY 4: Tick Momentum Burst Detection
            ...
            if (tick_burst_status['burst_length'] >= 4 and 
                ...
                if 0.15 <= entry <= 0.85:
                    # V14: EV check before entry
                    ev_pass, ev_result, prob_result = self._check_ev(market_type, direction, entry, 'tick_burst', elapsed)
                    if not ev_pass:
                        self.last_kalshi_bids[market_type] = kalshi['yes_bid']
                        return
                    ...
                    if pos_id is not None:
                        ...
                        # Adjust position size based on MTF alignment
                        if mtf_ratio < 1.0:
                            pos = self.trader.positions[-1]
                            original_size = pos['size']
                            pos['size'] = max(5, int(pos['size'] * 0.5))  # Half size
                        ...
                        self._dual_log(f"[{elapsed:5d}s] ⚡ {market_type.upper()} TICK_BURST: {tick_burst_status['burst_length']} consecutive {tick_burst_status['direction'].upper()} ticks, total +{tick_burst_status['total_move_pct']:.2f}% → {direction}@{entry:.2f} (MTF={mtf_ratio:.1%}) ev=${ev_result['ev']:.3f}")
                        self.signals.append({'type': 'tick_burst', 'market': market_type, 'time': elapsed, 'ev': ev_result['ev']})
        ...
        # STRATEGY 5: Cross-Exchange Momentum Clustering
            ...
            if (cluster_direction is not None and 
                ...
                if not delay_arb_would_trigger:
                    ...
                    if 0.15 <= entry <= 0.85:
                        # V14: EV check before entry
                        ev_pass, ev_result, prob_result = self._check_ev(market_type, direction, entry, 'momentum_cluster', elapsed)
                        if not ev_pass:
                            self.last_kalshi_bids[market_type] = kalshi['yes_bid']
                            return
                        ...
                        if pos_id is not None:
                            ...
                            self._dual_log(f"[{elapsed:5d}s] 🔄 {market_type.upper()} CLUSTER: {num_agreeing}/4 exchanges {cluster_direction.upper()} avg +{cluster_strength:.3%} → {direction}@{entry:.2f} (MTF={mtf_ratio:.1%}) ev=${ev_result['ev']:.3f}")
                            self.signals.append({'type': 'momentum_cluster', 'market': market_type, 'time': elapsed, 'strength': cluster_strength, 'num_agreeing': num_agreeing, 'ev': ev_result['ev']})
            ...
            if imbalance_score is not None:
                ...
                if total_volume > min_volume_threshold:
                    ...
                    if imbalance_score > 0.3:
                        ...
                        entry = k_ask
                        # V14: EV check before entry
                        ev_pass, ev_result, prob_result = self._check_ev(market_type, direction, entry, 'orderbook_imbalance', elapsed)
                        if ev_pass:
                            ...
                            if pos_id is not None:
                                ...
                                self._dual_log(f"[{elapsed:5d}s] 📊 {market_type.upper()} OB_IMBALANCE: score={imbalance_score:+.2f} bid_vol={bid_vol} ask_vol={ask_vol} → {direction}@{entry:.2f} ev=${ev_result['ev']:.3f}")
                                self.signals.append({'type': 'orderbook_imbalance', 'market': market_type, 'time': elapsed, 'score': imbalance_score, 'ev': ev_result['ev']})
        ...
        # STRATEGY 7: Delay Arbitrage — Original profitable strategy
            ...
            if abs(momentum_1m) > 0.20 and abs(kalshi_chg) < 5:
                ...
                if 0.15 <= entry <= 0.85:
                    # V14: EV check before entry
                    ev_pass, ev_result, prob_result = self._check_ev(market_type, direction, entry, 'delay_arb', elapsed)
                    if not ev_pass:
                        self.last_kalshi_bids[market_type] = kalshi['yes_bid']
                        return
                    ...
                    if pos_id is not None:
                        ...
                        self._dual_log(f"[{elapsed:5d}s] 🎯 {market_type.upper()} DELAY_ARB: BTC {momentum_1m:+.3f}% K {kalshi_chg:+.1f}% → {direction}@{entry:.2f} (MTF={mtf_ratio:.1%}) ev=${ev_result['ev']:.3f}")
                        self.signals.append({'type': 'delay_arb', 'market': market_type, 'time': elapsed, 'ev': ev_result['ev']})
    ...
    async def run(self):
        try:
            ...
            log_timestamped = open(f'/home/clawdbot/clawd/btc-arbitrage/logs/rt_v14_{ts}.log', 'w')
            log_live = open(f'/home/clawdbot/clawd/btc-arbitrage/logs/rt_v14_live.log', 'w')
            ...
            header = f"\n{'='*70}\nv14 - Probability-First Trading\n{'='*70}\n" + \
                    f"Duration: {self.duration//60}min | Start: {datetime.now().strftime('%H:%M:%S')}\n" + \
                    f"V14 Changes: Probability scoring + EV gating (MIN_EV=${MIN_EV})\n" + \
                    f"  Fixed VOL_REGIME: BTC {VOL_REGIME_BTC*100:.1f}% / ETH {VOL_REGIME_ETH*100:.1f}% (soft filter)\n" + \
                    f"  Fixed stops: ATR {ATR_STOP_MULTIPLIER}x, floor BTC ${ATR_STOP_FLOOR_BTC} / ETH ${ATR_STOP_FLOOR_ETH}\n" + \
                    f"  Flash sniper SL: {FLASH_SNIPER_SL_PCT*100:.0f}% (was 10%)\n" + \
                    f"  Time-based stop relaxation for profitable trades\n" + \
            ...
            while time.time() < end_time and not self.graceful_shutdown:
                ...
                for pos in self.trader.get_open():
                    ...
                    if strategy == 'flash_sniper':
                        ...
                        # V14: Stop loss: price drops another 15% below entry (was 10% — too tight)
                        if not should_close and loss_per_contract >= pos['entry'] * FLASH_SNIPER_SL_PCT:
                            should_close = True
                            reason = f"FLASH_SL(dropped {FLASH_SNIPER_SL_PCT*100:.0f}%+ below entry)"
                    ...
                    # ===== ALL OTHER STRATEGIES: Standard exit rules =====
                        ...
                        # V14: Time-based stop relaxation — if trade is in profit, widen stops progressively
                        if profit_per_contract > 0 and hold_time > 30:
                            # In profit for >30s: widen stop by 20% per 30s, up to 2x
                            relaxation_factor = min(2.0, 1.0 + (hold_time - 30) / 150.0)
                            stop_dollar = stop_dollar * relaxation_factor
                        ...
                        # Hard stop: still protect against initial loss (V14: wider ATR-based)
                ...
                if time.time() - last_status > 60:
                    ...
                    st3 = f"  {atr_str} {rsi_str} {ema_str} {ls_str}"
                    
                    # V14: Probability estimate in status line
                    try:
                        btc_indicators_dict = self._gather_indicators('15m')
                        btc_prob = self.scorer.estimate_probability(btc_indicators_dict, 'BTC')
                        ev_entries = sum(1 for e in self.ev_log if e['entered'])
                        ev_skips = sum(1 for e in self.ev_log if not e['entered'])
                        st4 = f"  [V14] P(BTC↑)={btc_prob['prob_up']:.3f} conf={btc_prob['confidence']:.2f} | EV decisions: {ev_entries} entered, {ev_skips} skipped"
                    except Exception:
                        st4 = f"  [V14] EV decisions: {sum(1 for e in self.ev_log if e['entered'])} entered, {sum(1 for e in self.ev_log if not e['entered'])} skipped"
    ...
    def _summary(self):
        ...
        summary = f"\n{'='*70}\nSUMMARY v14 — Probability-First Trading\n{'='*70}\nDuration: {elapsed//60}min | Mode: {self.poller.mode} | Signals: {len(self.signals)}\nTrades: {len(self.trader.trades)} | P&L: ${self.trader.pnl:+.2f} | Flash Crashes: {flash_count}\n"
        
        # V14: EV decision summary
        ev_entries = sum(1 for e in self.ev_log if e['entered'])
        ev_skips = sum(1 for e in self.ev_log if not e['entered'])
        total_ev_decisions = len(self.ev_log)
        summary += f"\nV14 EV Decisions: {total_ev_decisions} total ({ev_entries} entered, {ev_skips} skipped)\n"
        if ev_entries > 0:
            avg_ev_entered = sum(e['ev'] for e in self.ev_log if e['entered']) / ev_entries
            summary += f"Avg EV on entries: ${avg_ev_entered:.4f}\n"
        if ev_skips > 0:
            avg_ev_skipped = sum(e['ev'] for e in self.ev_log if not e['entered']) / ev_skips
            summary += f"Avg EV on skips: ${avg_ev_skipped:.4f}\n"
        summary += f"MIN_EV threshold: ${MIN_EV}\n"
        summary += f"VOL_REGIME: BTC {VOL_REGIME_BTC*100:.1f}% / ETH {VOL_REGIME_ETH*100:.1f}% (soft)\n"
        summary += f"ATR_STOP: {ATR_STOP_MULTIPLIER}x, floors BTC ${ATR_STOP_FLOOR_BTC} / ETH ${ATR_STOP_FLOOR_ETH}\n"
        ...
        ts = datetime.now().strftime('%Y%m%d_%H%M%S')
        with open(f'/home/clawdbot/clawd/btc-arbitrage/data/rt_v14_{ts}.json', 'w') as f:
            json.dump({'version': 'v14', 'duration_min': elapsed//60, 'pnl': self.trader.pnl, 
                      'signals': self.signals,
                      ...
                      'v14_config': {
                          'min_ev': MIN_EV,
                          'vol_regime_btc': VOL_REGIME_BTC,
                          'vol_regime_eth': VOL_REGIME_ETH,
                          'atr_stop_multiplier': ATR_STOP_MULTIPLIER,
                          'atr_stop_floor_btc': ATR_STOP_FLOOR_BTC,
                          'atr_stop_floor_eth': ATR_STOP_FLOOR_ETH,
                          'flash_sniper_sl_pct': FLASH_SNIPER_SL_PCT,
                      },
                      'ev_log_summary': {
                          'total_decisions': len(self.ev_log),
                          'entries': ev_entries,
                          'skips': ev_skips,
                          'recent_decisions': self.ev_log[-50:] if self.ev_log else [],
                      },
...
async def main():
    ...
    engine = TradingEngine(feed, poller, okx_feed, duration_min=duration)
    ...
    try:
        ...
        okx_task.cancel()
        try:
            await asyncio.gather(feed_task, poller_task, okx_task, return_exceptions=True)
        except:
            pass
