#!/bin/bash
# V8 continuous runner - keeps restarting after each 8h session
# For 48h+ data collection
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PROJECT_ROOT="$(dirname "$SCRIPT_DIR")"
PID_FILE="/tmp/rt_v8_pid.txt"
SESSION_MIN=${1:-480}  # each session 8h
MAX_CRASH_RESTARTS=3   # max restarts per session on crash
CRASH_COUNT=0

cd "$PROJECT_ROOT"

echo "🚀 V8 Continuous Runner"
...
while true; do
    ...
    echo -e "\n[$(date '+%Y-%m-%d %H:%M:%S')] 启动 V8 session (${SESSION_MIN}min)"
    
    python3 src/realtime_paper_trader_v8.py $SESSION_MIN &
    pid=$!
    echo $pid > "$PID_FILE"
    echo "✅ PID: $pid"
    ...
    if [ $exit_code -eq 0 ] && [ $session_elapsed -ge $((expected - 120)) ]; then
        # Normal completion - reset crash count, start next session
