#!/usr/bin/env python3
"""
Real-time Paper Trader v11 - Trend Filter - WebSocket + Enhanced Features + Kelly Criterion + Clustering
...
class TradingEngine:
    def __init__(self, feed, poller, duration_min=480):
        ...
        self.state_file = '/home/clawdbot/clawd/btc-arbitrage/data/rt_v11_state.json'
    ...
    def _check_crash_regime(self, market_type):
        """Day 1 Fix: Check if BTC/ETH has dropped >5% in last 4 hours.
        If so, disable YES (long) trades — going long in a crash is a guaranteed loser.
        Returns (is_crash, pct_drop).
        """
        if market_type in ('15m', 'btc_daily'):
            history = self.feed.price_history
        else:
            history = self.feed.eth_price_history
    ...
    def _process_market(self, market_type, elapsed, brti, momentum_1m):
        ...
        # STRATEGY 3: Steam Move Detection - Smart money following (V11: with trend filter)
            ...
            if steam_result['steam_detected'] and steam_result['direction']:
                ...
                # Day 1 Fix: Crash regime filter — disable YES trades when BTC/ETH dropped >5% in 4h
                is_crash, crash_pct = self._check_crash_regime(market_type)
                if is_crash and direction == 'YES':
                    self._dual_log(f"[{elapsed:5d}s] 🚫 {market_type.upper()} CRASH REGIME: {crash_pct:+.1f}% in 4h — blocked YES@{entry:.2f}")
                    self.last_kalshi_bids[market_type] = kalshi['yes_bid']
                    return
    ...
    async def run(self):
        try:
            ...
            log_timestamped = open(f'/home/clawdbot/clawd/btc-arbitrage/logs/rt_v11_{ts}.log', 'w')
            log_live = open(f'/home/clawdbot/clawd/btc-arbitrage/logs/rt_v11_live.log', 'w')
            self.log_files = [log_live, log_timestamped]
            
            self._setup_signal_handlers()
            
            header = f"\n{'='*70}\nv11 - Trend Filter + Multi-Market Support\n{'='*70}\n" + \
    ...
    def _summary(self):
        ...
        ts = datetime.now().strftime('%Y%m%d_%H%M%S')
        with open(f'/home/clawdbot/clawd/btc-arbitrage/data/rt_v11_{ts}.json', 'w') as f:
