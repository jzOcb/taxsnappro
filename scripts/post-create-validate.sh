...
# Checks for function duplication and missing imports from established modules
# Usage: bash scripts/post-create-validate.sh <new_file_path>
...
if [ -z "$NEW_FILE" ] || [ ! -f "$NEW_FILE" ]; then
    ...
    echo "Usage: bash scripts/post-create-validate.sh <file_path>"
    exit 1
fi
