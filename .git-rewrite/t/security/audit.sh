...
# 1. Open ports
echo ""
echo "📡 [1/7] Open ports (listening on all interfaces):"
...
echo ""
echo "🚨 [4/7] Failed SSH attempts (last 24h):"
...
if [ -f "$OPENCLAW_JSON" ]; then
    if grep -qE '(sk-|key-|token)' "$OPENCLAW_JSON" 2>/dev/null; then
        ...
        echo "     Move secrets to env vars or /opt/clawdbot.env"
    else
...
echo ""
echo "Done. Run 'bash harden.sh' to apply fixes."
