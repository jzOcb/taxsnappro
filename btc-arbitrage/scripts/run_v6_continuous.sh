#!/bin/bash
# V6 continuous runner - keeps restarting after each 8h session
...
while true; do
    ...
    echo -e "\n[$(date '+%Y-%m-%d %H:%M:%S')] 启动 V6 session (${SESSION_MIN}min)"
    
    python3 src/realtime_paper_trader_v6.py $SESSION_MIN &
    pid=$!
    echo $pid > "$PID_FILE"
    echo "✅ PID: $pid"
