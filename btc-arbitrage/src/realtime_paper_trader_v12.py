#!/usr/bin/env python3
"""
Real-time Paper Trader v12 - Quant Enhanced - Advanced Technical Analysis + All V11 Features
Upgraded from v11 with:
1. TechnicalIndicators class (ATR, RSI, EMA, Bollinger Bands)
2. Bootstrap historical data on startup (300x 1-min candles)
3. OKX derivatives data feed (funding rates, open interest, long/short ratios)
4. Enhanced entry filters (volatility regime, RSI, multi-timeframe, crowd sentiment)
5. ATR-based adaptive stop-loss (replaces fixed dollar stops)
6. Improved logging and status displays

Key Features (All V11 + New):
- Technical indicators: ATR(14), RSI(14), EMA(5/20), Bollinger Bands
- Volatility regime detection (high/low/normal vol)
- Multi-timeframe trend confirmation (requires 2/3 aligned)
- RSI overbought/oversold filter
- OKX derivatives sentiment data (funding, OI, long/short ratio)
- ATR-based adaptive stops (1.5x-2.0x ATR)
- Startup warmup reduced to 30s (vs 120s) due to historical bootstrap
- All existing V11 strategies preserved
"""
...
class TradingEngine:
    def __init__(self, feed, poller, okx_feed, duration_min=480):
        ...
        self.state_file = '/home/clawdbot/clawd/btc-arbitrage/data/rt_v12_state.json'
    ...
    async def run(self):
        try:
            ...
            log_timestamped = open(f'/home/clawdbot/clawd/btc-arbitrage/logs/rt_v12_{ts}.log', 'w')
            log_live = open(f'/home/clawdbot/clawd/btc-arbitrage/logs/rt_v12_live.log', 'w')
            ...
            header = f"\n{'='*70}\nv12 - Quant Enhanced\n{'='*70}\n" + \
                    ...
                    f"New Features: Technical indicators (ATR, RSI, EMA, BB), volatility regime\n" + \
                    f"              detection, multi-timeframe confirmation, OKX sentiment\n" + \
                    f"              data, ATR-based adaptive stops, enhanced filters\n" + \
    ...
    def _summary(self):
        ...
        ts = datetime.now().strftime('%Y%m%d_%H%M%S')
        with open(f'/home/clawdbot/clawd/btc-arbitrage/data/rt_v12_{ts}.json', 'w') as f:
