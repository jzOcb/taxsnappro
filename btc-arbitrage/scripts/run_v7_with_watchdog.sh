...
RESTART_FLAG="/tmp/rt_v7_restart.flag"
MAX_RESTARTS=3
DURATION_MIN=${1:-480}
...
echo "🚀 v7 — Research-Validated High-Frequency Validation Mode"
echo "Duration: ${DURATION_MIN}min | Max Restarts: $MAX_RESTARTS"
echo "========================================"

while [ $restart_count -lt $MAX_RESTARTS ]; do
    echo -e "\n[$(date '+%Y-%m-%d %H:%M:%S')] 启动 v7 (attempt $((restart_count+1))/$MAX_RESTARTS)"
    
    python3 src/realtime_paper_trader_v7.py $DURATION_MIN &
    pid=$!
    echo $pid > "$PID_FILE"
