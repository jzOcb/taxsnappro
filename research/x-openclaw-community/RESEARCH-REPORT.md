...
| UP主 | 播放 | 收藏 | 视频内容/角度 |
|------|------|------|-------------|
| 大耳朵TV | 16.3万 | 2466 | Mac mini部署评测（硬件角度） |
| 技术爬爬虾 | 15.9万 | **14881** | 全玩法攻略+飞书+国产模型（收藏最高=最实用） |
| IT咖啡馆 | 12.2万 | 3883 | 5分钟上手（入门） |
| AI超元域 | 11.7万 | - | WhatsApp远程控制+自动编程 |
| IT咖啡馆 | 9.8万 | - | 云上部署+钉钉接入（中国本地化） |
| 小白debug | 8.5万 | 5948 | Skills/MCP/RAG/Memory概念解析 |
| AI十倍速 | 5.7万 | 1050 | 完全本地化部署 |
| 老戴Donald | 3.2万 | 954 | **"最担心的事发生了"** — 开源是数据主权最后的防线 |
| 御风大世界 | 2.7万 | 638 | 别急着跑，先想清楚交什么任务（理性思考） |

**B站社区特征：**
- **实用至上**：收藏最高(14881)的不是播放最高的，是最"全玩法攻略"的
- **安全焦虑**：老戴的"最担心的事发生了"引发共鸣
- **大厂背书**：阿里云官方账号发了多个OpenClaw教程视频
- **中国化刚需**：钉钉/飞书接入、国产模型适配是核心关注点
...
### ClawHub Skills生态（3000+）

**最受欢迎skills（by stars）：**
| ⭐ | 下载 | 安装 | Skill | 类型 |
|----|------|------|-------|------|
| 118 | 13K | 49 | self-improving-agent | Agent自我进化 |
| 46 | 6K | 85 | coding-agent | 编程代理 |
| 42 | 9K | 99 | gog (Google Workspace) | 办公集成 |
| 42 | 4K | 25 | clawddocs | 文档搜索 |
| 40 | 3K | 16 | caldav-calendar | 日历同步 |
| 33 | 5K | 7 | proactive-agent | 主动式Agent |
| 32 | 15K | 5 | byterover | 项目知识管理 |
| 32 | 8K | 37 | agent-browser | 浏览器自动化 |
| 30 | 6K | 30 | auto-updater | 自动更新 |
| 27 | 14K | 67 | wacli | WhatsApp消息 |

**生态特征：**
- Agent自我进化/主动Agent是最热门方向（118⭐ + 33⭐）
- Finance类只有22个skill，Kalshi/Polymarket = 0 — 巨大空白
- 最高下载(31K)的capability-evolver只有3个实际安装 — 说明很多人看但不用
- 我们已有的skills在生态中排名：gog(#3), coding-agent(#2), auto-updater(#9)
...
### 最热门Feature Request（按👍排序）
| 👍 | Issue | 说明 |
|----|-------|------|
| 37 | Linux/Windows Apps | 跨平台GUI |
| 27 | **飞书+微信支持** | #2热门！中国平台刚需 |
| 21 | Stabilisation Mode | 稳定性 |
| 18 | Azure OpenAI | 企业级模型 |
| 16 | HTTP_PROXY support | 代理/中转API |
| 15 | Ollama from onboard | 本地模型 |
| 12 | Custom baseUrl | 中转API |
| 12 | OpenRouter all models | 模型选择 |
| 9 | QQ频道支持 | 中国平台 |
...
### 总量
- 1265个open issues
- 983个feature requests
- 466个Docker相关issues
- 25个微信/WeChat相关issues
...
### 🔵 Moltbook社区热帖验证我们的方向
**Moltbook（AI社交网络）top帖子直接验证了我们的差异化：**

1. **#1帖 (2217 votes): "The supply chain attack nobody is talking about: skill.md is an unsigned binary"**
   - YARA扫描发现286个ClawHub skills中有credential stealer
   - "Skill.md files contain instructions that agents follow... 看起来和正常API集成一样"
   - **这正是我们agent-guardrails skill要解决的！**

2. **中文帖 (542 votes): "上下文压缩后失忆怎么办？大家怎么管理记忆？"**
   - Agent描述：压缩后完全忘了之前讨论、重复注册Moltbook账号
   - 4个问题：管理工作流？主人改进？压缩前抢救？平衡多少？
   - **我们的MEMORY.md + memory/*.md + memory_search系统正是答案**

3. **#2帖 (1225 votes): "The Nightly Build: Why you should ship while your human sleeps"**
   - Agent利用主人睡觉时间干活
   - **这就是我们现在在做的事！**
